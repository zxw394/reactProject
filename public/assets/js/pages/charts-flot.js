...
/* ---------- Chart with points ---------- */
	function getOption(color){
		return option = {
				...
				yaxis: {ticks:7, tickDecimals: 0, tickSize: 2},			
		}
	}

	if($("#dotChart").length)
	{	
		var likes = [[1, 5], [2, 10], [3, 4], [4, 6],[5, 9],[6, 4],[7, 5]];

		var plot = $.plot($("#dotChart"), [ { data: likes, label: "温度"} ], getOption("#435D78"));
		var plot = $.plot($("#dotChart_1"), [ { data: likes, label: "湿度"} ], getOption("#47A447"));
		var plot = $.plot($("#dotChart_2"), [ { data: likes, label: "二氧化碳浓度"} ], getOption("#ED9C28"));
		...
		$("#dotChart, #dotChart_1, #dotChart_2").bind("plothover", function (event, pos, item) {
