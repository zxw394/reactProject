...
/*------- Gauge -------*/
	...
	$("#eventList").hover(
	function(){
		clearInterval(time);//停止滚动
	},
	function(){
        time = setInterval(function(){   
            var div = $("#eventList");   
            var rowHeight = div.find(".row:last").height();//计算最后一个row的高度
            var rowClone = div.find(".row:last")
            div.animate({marginTop : rowHeight+"px", height : (rowHeight*3)+"px"},1000,function(){   
            	div.find(".row:last").prependTo(div).fadeIn(2000);   
//            	div.find(".row:first").hide();   
            	div.css({marginTop:0,height:rowHeight*4+"px"});   
//            	div.find(".row:first").;   
            });
    	},3000); 		
	}).trigger("mouseleave")
